...
"""
    ...
    def _build_analysis_materials(self, scout_data: Dict, analyst_data: Dict) -> str:
        ...
        Scout: 市场层面（爆火原因、趋势、社区热度、竞争优势、风险）
        Analyst: 技术层面（部署难度、代码质量、Issue健康度）
        """
        sections = []
    ...
    def _format_scout_data(self, data: Dict) -> str:
        if not data:
            return ""
        ...
        if trend:
            lines.append(f"技术趋势: {trend}")

        advantage = data.get("competitive_advantage", "")
        if advantage:
            lines.append(f"竞争优势: {advantage}")

        sentiment = data.get("community_sentiment", {})
        if sentiment:
            heat = sentiment.get("heat_level", "unknown")
            atmosphere = sentiment.get("atmosphere", "unknown")
            topics = sentiment.get("key_topics", [])
            lines.append(f"社区热度: {heat} ({atmosphere})")
            if topics:
                lines.append(f"讨论话题: {', '.join(topics)}")

        concerns = data.get("potential_concerns", [])
        if concerns:
            lines.append(f"潜在风险: {', '.join(concerns)}")

        search_meta = data.get("search_metadata", {})
        if search_meta:
            mentions = search_meta.get("total_mentions", 0)
            has_external = search_meta.get("has_external_data", False)
            if mentions > 0 or has_external:
                lines.append(f"外部讨论: 总提及{mentions}次{'(含HackerNews/Reddit)' if has_external else ''}")

        return "\n".join(lines) if lines else ""

    def _format_analyst_data(self, data: Dict) -> str:
        if not data:
            return ""

        tech_review = data.get("tech_review", data)
        if not tech_review:
            return ""
        ...
        runnability = tech_review.get("runnability", {})
        if runnability:
            difficulty = runnability.get("difficulty", "unknown")
            assessment = runnability.get("assessment", "")
            missing = runnability.get("missing_configs", [])
            lines.append(f"可运行性: {difficulty}")
            if assessment:
        ...
        structure = tech_review.get("code_structure", {})
        if structure:
            level = structure.get("engineering_level", "unknown")
            assessment = structure.get("assessment", "")
            lines.append(f"代码质量: {level}")
            if assessment:
                lines.append(f"  评估: {assessment}")

        issues = tech_review.get("issue_analysis", {})
        if issues:
            bug_density = issues.get("bug_density", "unknown")
            warning = issues.get("critical_warning", "")
            active = issues.get("author_active", False)
            lines.append(f"Issue健康度: Bug密度-{bug_density}, 作者活跃-{ '是' if active else '否'}")
            if warning and warning != "无":
                lines.append(f"  严重警告: {warning}")

        recommendation = tech_review.get("recommendation", "")
        reasoning = tech_review.get("reasoning", "")
        if recommendation:
