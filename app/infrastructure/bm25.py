"""
BM25 关键词检索模块。
使用 rank-bm25 实现 BM25 检索，支持中英混合分词。
"""
...
class BM25SearchEngine:
    ...
    def _init_jieba(self):
        ...
        try:
            with warnings.catch_warnings():
                # jieba 内部依赖 pkg_resources，会触发弃用告警；这里仅抑制该已知噪声。
            ...
            logger.info("jieba 分词器初始化完成（version=%s）", getattr(jieba, "__version__", "unknown"))
        except ImportError:
    ...
    def build_index(self, documents: List[Dict]) -> None:
        ...
        if self.corpus and self._bm25_available:
            self.bm25 = BM25Okapi(self.corpus)
            logger.info("BM25 索引构建完成，共 %s 个文档", len(documents))
        elif self.corpus:
            self.bm25 = None
            logger.warning("rank_bm25 不可用，BM25 将降级为词重叠打分检索。")
        else:
    ...
    def _tokenize(self, text: str) -> List[str]:
        ...
        stopwords = {
            "的", "是", "在", "有", "和", "了", "与", "及", "等", "中", "对", "这", "那", "我", "你", "他",
            "她", "它", "们", "一个", "可以", "使用", "进行", "通过", "支持", "提供",
        ...
        if self._jieba_initialized:
            ...
            for word in english_words:
                word = word.lower()
                if (word in tech_terms or len(word) >= 3) and word not in stopwords:
                    tokens.append(word)
        else:
    ...
    def search(self, query: str, top_k: int = 10) -> List[Dict]:
        ...
        if self.bm25 is not None:
            ...
            # Fallback scoring when rank_bm25 is unavailable:
            # score = overlap(query_tokens, doc_tokens) with mild tf weighting.
            ...
            for doc_tokens in self.corpus:
                ...
                for tok in query_set:
                    if tok in tf:
                        overlap_score += 1.0 + min(tf[tok], 3) * 0.2
